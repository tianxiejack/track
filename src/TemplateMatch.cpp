...
static void convert_UC2F(IMG_MAT_UCHAR	*src,	IMG_MAT_FLOAT	*dst)
{
	int i, j;
	unsigned char *psrc;
	float *pdst;
	...
	{
#pragma UNROLL(4)
		for(j=0; j<src->height*src->width; j++)
		{
			psrc = (unsigned char*)src->data_u8 + j;
			pdst = (float*)dst->data + j;
			*pdst = (float)(*psrc);
		}
	}
...
#pragma UNROLL(4)
		...
		{
			psrc = (unsigned char*)src->data_u8 + src->step[0]*j;
			pdst = (float*)dst->data + dst->step[0]*j;
			for(i=0; i<src->width; i++)
			{
				*pdst++ = (float)(*psrc++);
			}
			memset(pdst, 0x00, sizeof(float)*(dst->width-src->width));
		}
	}
}

static void convert_UC2F_ROI(IMG_MAT_UCHAR	*src,	IMG_MAT_FLOAT	*dst,	VALID_ROI	*src_roi, VALID_ROI	*dst_roi)
{
	int i, j;
	unsigned char *psrc;
	float *pdst;
...
static void crossCorr( const IMG_MAT_UCHAR img, const IMG_MAT_UCHAR _templ, IMG_MAT_FLOAT corr, 
								CSize corrsize, int ctype,	CPoint anchor, double delta, int borderType )
{
	const double blockScale = 4.5;
	const int minBlockSize = 256;
	IMG_MAT_UCHAR templ = _templ;
...
// calculate correlation by blocks
	for( i = 0; i < tileCount; i++ )
	{
		...
		IMG_MAT_UCHAR src0;//(img0, Range(y1, y2), Range(x1, x2));
		...
		cdst_roi.valid_w = bsz.width;		cdst_roi.valid_h = bsz.height;
		{
			...
			convert_UC2F_ROI(&src0, &dst1, &src0_roi, &dst1_roi);
...
static void Integral(IMG_MAT_UCHAR _img, IMG_MAT_FLOAT _sum, IMG_MAT_FLOAT _sqsum, int cn)
{
	int x, y, k;
	int nWidth, nHeight;
	...
	nWidth = _img.width;
	nHeight = _img.height;
	pImg = (unsigned char *)_img.data_u8;
...
#pragma UNROLL(4)
	for( y = 0; y < nHeight; y++, pImg += imgStep - cn,	pSum += sumStep - cn, pSqSum += sqsumStep - cn )
	{
		for( k = 0; k < cn; k++, pImg++, pSum++, pSqSum++ )
		{
			float s = pSum[-cn] = 0;
			float sq = pSqSum[-cn] = 0;
			for( x = 0; x < nWidth; x += cn )
			{
				unsigned char it = pImg[x];
				float t, tq;
				s += it;
				sq += (float)it*it;
				t = pSum[x - sumStep] + s;
...
static void meanStdDev(IMG_MAT_UCHAR _img, float *mean, float *stddev, int cn)
{
	int i, j, k;
...
#pragma UNROLL(4)
		for( i = 0; i < len; i++, pImg += cn )
		{
			unsigned char v0 = pImg[0], v1 = pImg[1];
...
#pragma UNROLL(4)
		for( i = 0; i < len; i++, pImg += cn )
		{
			unsigned char v0 = pImg[0], v1 = pImg[1], v2 = pImg[2];
			s0 += v0; sq0 += (float)v0*v0;
			s1 += v1; sq1 += (float)v1*v1;
...
void matchTemplateUchar( IMG_MAT_UCHAR _img, IMG_MAT_UCHAR _templ, IMG_MAT_FLOAT _result, int method )
{
	...
	if( method == CR_TM_CCORR )
		return;
	{
		double invArea = 1./((double)_templ.height * _templ.width);
		...
		if( templNorm < FLT_EPSILON  && method == CR_TM_CCOEFF_NORMED )
		{
